...
export class BitgetClientService {
	...
	/** Close a partial position (one TP target) via market order. */
	async closePartialPosition(p: {
		...
		this.logger.log(`Partial close sent: ${p.symbol} ${p.side} qty=${sizeStr} orderId=${resp?.data?.orderId}`);
	}
	...
	async closeFullPosition(p: {
		...
		const closeSide = p.side === OrderSide.LONG ? 'sell' : 'buy';
		const sizeStr = this.truncateSize(p.symbol, p.qty);
		await client.futuresSubmitOrder({
			symbol: p.symbol,
			productType: 'USDT-FUTURES',
			marginCoin: 'USDT',
			marginMode: 'crossed',
		...
		this.logger.log(`Full close sent: ${p.symbol} ${p.side} qty=${sizeStr}`);
	}

	/** Update SL to break-even after TP1 hit. */
	async updateSL(p: {
		apiKey: string; apiSecret: string; passphrase: string;
		...
		try {
			...
			this.logger.log(`SL updated to ${priceStr} for ${p.symbol} ${holdSide}`);
		} catch (e: any) {
			this.logger.warn(`SL update failed: ${e.body ? JSON.stringify(e.body) : e.message}`);
			throw e;
		}
	}
